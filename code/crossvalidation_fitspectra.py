...
def get_data(flag,cv_ind):
  if glob.glob('normed_data.pickle'): # MOVE THIS WAY UP AND PICKLE metaall TOO
    ...
    file_in2.close()
    if logical_and(flag==1, len(cv_ind) =1): 
        dataall = np.delete(dataall, (cv_ind), axis = 1) 
        metaall = np.delete(metaall, (cv_ind), axis = 0) 
        predictors = np.delete(predictors, (cv_ind), axis = 0) 
    if logical_and(flag==1, len(cv_ind) >1): 
        for each in cv_ind:
          dataall = np.delete(dataall, (each), axis = 1) 
          metaall = np.delete(metaall, (each), axis = 0) 
          predictors = np.delete(predictors, (each), axis = 0) 
    bad3 = isinf(dataall[:,:,2])
    dataall[bad3,2] = 100000. 
 ...
 # bad = np.logical_or(np.isnan(sigma), np.isnan(y2new)) 
  ...
  dataall[bad2,2] = 100000. #note if infinity falls over later 
  bad3 = np.isinf(dataall[:,:,2])
  dataall[bad3,2] = 100000. #note if infinity falls over later 
  #bad3 = dataall[:,:,1] == 0. 
...
individuals = arange(0,554,1) 

flag = 1 
for cv_ind in range(0,554,1): 
  dataall, metaall, predictors, count = get_data(flag,cv_ind)
  ...
  params_one,Cinv_Params = getpar_one(coeffs, scatters, metaall, cv_ind,filename_in) 

# below = on globular clusters one at a time 
a = open('starsin_new_all_ordered.txt', 'r' ) 
al = a.readlines()
names = []
for each in al:
...
name_ind_all = [0] + list(sort(name_ind)) #+ [len(names)] 

# not implemneted yet but implement for teststar2_dataall = should have different name 
#for i in range(0,len(name_ind_all):
#        ind1 = arange(name_ind_all[i], name_ind_all[i+1])
#        name_gc = [unames]
#        dataall, metaall, predictors, count = get_data(flag,cv_ind)
#        blob = do_regressions(dataall, predictors)
#        coeffs = np.array([b[0] for b in blob])
...
#        chisqs = np.array([np.dot(b[2],b[2]) - b[3] for b in blob]) # holy crap be careful
#        scatters = np.array([b[4] for b in blob])
#        params,icovs_params = return_test_params(testfile,scatters,coeffs,name_gc[0])
        #params_one,Cinv_Params = getpar_one(coeffs, scatters, metaall, cv_ind,filename_in) 
