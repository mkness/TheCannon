...
Stars with \badstar\ flag set (in \aspcapflag) may have unphysical stellar parameters and commissioning stars are marked with ``C'', the fidelity of the commissioning stars is uncertain given their different LSF from survey test and training data. The velocity scatter ($\sigma_v$) from the \aspcap\ results as well as the \texttt{APOGEE TARGET2} flag (TARG2) are provided in these tables. We provide two columns in this table that reflect the labels of the test object with respect to the training set. We provide an 'extrapolation flag' EFLAG', which indicates if the test star lies outside of the label-space of the reference objects. We also determine a distance measurement, defined as the distance between the test star and the nearest reference object, normalised to the maximum distance, so values lie between 0 and 1. }

\begin{equation}
\textcolor{blue}{dist = |min(\frac{(T_{test} - T_{ref})^2}{\sigma^2_{T_{ref}}} + \frac{(g_{test} - g_{ref})^2}{\sigma^2_{g_{ref}}} + \frac{([Fe/H]_{test} - [Fe/H]_{ref})^2}{\sigma^2_{[Fe/H]_{ref}}})|}
\end{equation}
...
\begin{figure}[!h]
\centering
  \includegraphics[scale=0.26]{plots/aftersubmit/stdtest_T_normed.png}
    \includegraphics[scale=0.26]{./plots/aftersubmit/stdtest_g_normed.png}
    \includegraphics[scale=0.26]{./plots/aftersubmit/stdtest_feh_normed.png}
