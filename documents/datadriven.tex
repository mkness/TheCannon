...
One particular case in the \apogee context is the Pleiades cluster: it is the \textit{only} cluster in which any dwarf stars are observed;
 otherwise the reference objects are completely devoid of main sequence stars. Tests including additional labels (e.g. [$\alpha$/Fe]) 
 shows that the rms decreases across all labels with an additional label constraint. 

+\textbf{HWR: well, this sentence is an important teaser. Suggestions: elaborate or leave out.. or say, as initial test beyond the scope of this paper shows.MKN: I like it in.}
