...
obtain \texit{rms} differences of $\delta\teff< 100$~K, $\delta\logg< 0.2$~dex
...
In the application of \tc\ to  \apogee, we start with the \aspcapstar\ and \apstar\ files of DR10,
which are reduced and resampled. 
The \apstar\  data are not pseudo-continuum normalised by \apogee\,
...
MKN: ONCE AGAIN THIS SECTION NAME MAKES NO SENSE: THIS IS THE TITLE OF THE WHOLE PAPER! DWH: Is it better now with the removal of the word ``Illustrative''? This is a subtle but important difference. I am not entirely sure how to make it clear APOGEE is an example but satisfy that you think it makes sense. On that note, is it a little confusing having a title "specifics of the APOGEE dataset" in section 2.3, as a lot of the info outside of that is also specific to the APOGEE dataset"?

MKN: THIS PARAGRAPH (below) ISN'T THE WAY TO START -- THIS DESCRIBES THE CROSS-VALIDATION, WHICH IS TWO SUBSECTIONS DOWN.  CAN YOU MOVE THIS THERE AND OPEN THIS SECTION BY SAYING THAT WE ARE GOING TO TRAIN THE QUADRATIC MODEL ON THE THREE LABELS WE HAVE CHOSEN USING THE TRAINING SET DESCRIBED ABOVE AND APPLY IT TO EVERYTHING? Yes have started doing this you probably still object to how much info is there I am not done with editing. 

We now illustrate how \tc\ works in practice, using \apogee\ DR10 data. We first train the quadratic model in Equation~\ref{eq:quadinlabels} using the reference data and three labels chosen as described in Section~\ref{sec:ApogeeRefLabels}. We then apply this model to all of the DR10 continuum normalised data, using continuum normalised \aspcapstar\ spectra described in Section~ \ref{sec:ApogeeContinuum}. We use a leave-one-out cross-validation test to explore which complexity the spectral model must have and how comprehensive the set of reference objects should be. We then proceed with the same set of reference objects in the label transfer to the entire DR10 in the test step. In particular, we also apply the test step to spectra from individual APOGEE visits that have far shorter exposure times, and hence lower SNR than the co-added spectra in DR10, in order to explore and illustrate how well \tc\ does at modest SNR (with appropriate continuum fitting).
...
An exception where this basic implementation would prove extremely useful is for any front-end to other methodologies of label-determination and this very simple version of the regression analysis is extremely fast for determining baseline stellar labels. 
% I am putting this in because at present a lot of surveys use some "fast" method first to get a first pass estimate to input to their pipelines - this might get their attention as this is *a lot* faster than what they are currently doing I believe. Someone commented this out I am putting it back. 
...
wavelength coverage (though not signal-to-noise) to the data on the survey objects that
require labeling.
...
in other wavelength regions, the method don't require---even
...
Although the primary objective of \tc\ was to free stellar parameter
determination and chemical abundance tagging from physical stellar
models, it is also the case that \tc\ is very fast, computationally.
...
...more generally what to do if we don't believe the labels?

...a generative model of spectra \emph{and} labels!

% HOGG: this sentence needs to go in some part of the discussion about the advantages we will get when we go beyond max-like to a probabilistic generative model
...
% HOGG: the following needs to move to a ``we use the whole spectrum'' paragraph?
% \tc\ is therefore a tool for capitalizing on large surveys and the
% (enormous) information content in the data itself, circumventing
% problems with simplified stellar models or uncertainties in line lists
% in poorly studied wavelength regions.
