...
and abundances (collectively ``stellar labels''), using not stellar models but rather a
...
wavelength coverage (though not necessarily signal-to-noise) to the data on the 
...
structure and photospheres, we do not rely on explicit physical models
for the spectra. \tc\  is based on the premise that spectra of stars with the same labels
look the same, and that spectra vary smoothly with changing labels. 
This makes it possible to propose a simple model for the spectrum as a function of the
lables, and fix this model in the straining step operating on the spectra of the reference objects.
In the subsequent test step, that same model can assign labels (and their uncertainties) to all
other objects in the survey.

In a first of \tc, on the \apogee\ DR10 data, we focussed on three basic labels, \teff, \logg, \feh , and 
derived them for basically all of the 52,000 stars, based on a training step that involved only 543 reference objects, i.e. 1\% of the survey.  Remarkably, \tc ~'s label transfer results in stellar parameter and metallicites that are as precise and accurate as those derived from ASPCAP. In addition, \tc\ appears to produce at least in this present circumstance plausible labels for 6000 main sequence stars in \apogee, even though only $\sim 60$ main sequence stars -- all members of the Hyades cluster -- were used in the training step. 
It is also remarkable that  the \logg - \teff\  diagram of Fig.\ref{fig:iso2} shows basically no stars outside
the physically plausible regime, although \tc\ knows nothing about stellar evolution save the training step.

Our application to \apogee\  illustrated a number of further strengths and practical advantages of such an approach. First, \tc\ is computationally very fast. It  trains fast and then delivers labels the 56,000 stars of the
...
attempt at code optimization. This is because \tc only involves linear algebra and the well-behaved optimization of a few parameters with an analytic model for the spectrum. 
...
The rms difference between  ASPCAP labels from spectra with $SNR\ge150$ an \tc\ labels
for the same stars from $SNR\sim 50$ survey spectra is only 30\% larger than the ASPCAP error bars. 
\tc exploits the information at all pixels and certainly the labels \teff, \logg, \feh\  affect many different parts of the spectra. How this behavious scales to more extensive label sets, encompassing e.g. individual abundances, remains to be seen. Part of the reason for this good behavior at low SNR is that
...
pixels that have near-unity flux, in initially normalized spectra, {\it and} show little variation with 
label changes. Those pixels are, conceptually and practically, good approximations to pixels to which to fit a smooth continuum. Our initial application to \apogee spectra shows that biased continuum fits spectra would be the main source of poor label estimates from lower SNR spectra using \tc , and may well also be for label estimates based on physical models. 

Our initial application of \tc\ to \apogee DR10 data, however, als brought to the fore and illustrated a number of important approximations and limitations.
...
These approximations revolve around the (reference) labels on the one hand, and on the choice 
of the spectral model on the other hand. 

Three important issues arise around labels. 1) The labels are assumed to be perfectly known, 
...
space comprehensively, especially if one considers high-dimensional label spaces (cf. \apogee 's DR12
published 16 labels per star). 3) Any choice for the dimensionality of the label space, 3D in our sample application, will be incomplete. Clearly, stars with identical \teff, \logg, \feh may have different spectra,
because they differ in $[\alpha/Fe]$ or $v_{rot}$.

The general approach to first , but also third issue is to expand the scope of the model.
...
$p(f_\lambda\given\starlabelvec,\set{\theta}_\lambda, s_\lambda^2)$ (cf Eq.\ref{eq:like}).
...
One would then, however, have to optimize
$\set{\theta}_\lambda$ for \textit{all} $\lambda$ pixels and the labels $\starlabelvec_n$
for \textit{all} $n$ reference objects simultaneously. Missing labels among reference
objects could then be treated pragmatically as having large uncertainties.

Thinking of \tc\ as a model for both the spectral intensities and the labels,
also shows how the (much more limited) external information on the labels of the survey
...
encorporates the entire survey. The difference between reference and survey objects
...
In this initial implementation of \tc\ we restricted ourselves to producing only a the maximum-likelihood estimates for both the $\set{\theta}_\lambda$ in the training step, and the $\starlabelvec_n$
in the test step, with errors only coming from the inverse covariance matric at that point.
But a full inference would be expensive, especially at test time
...
Note that in the application ot \tc\ to \apogee\ we did deal with systematic errors in the reference
...
The other two issues raised above are linked: 
...
Conceptually, it is trivial to extend \tc\ to even much larger numbers of labels. 
...
With pixel-by-pixel polynomial \textit{Ansatz} for the spectral model $\set{\theta}_\lambda$
...
but we arribed at that choice by empirical experimentation with this particular data set. 
This is a choice  can be generalized.
...
spectral pixel independently, which they plausibly are only in their noise propeties.
...
{\bf HWR ended here; dinner-time Jan 14}
