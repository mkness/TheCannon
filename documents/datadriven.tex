...
(from running \tc\ itself iteratively) are not strongly affected by absorption lines.
This constitutes a data-driven method for finding continuum pixels.
We find that this method 
leads to very small systematic dependence of the spectra on SNR.
...
we determine the continuum by fitting a low-order Chebyshev polynomial to these continuum pixels,
 treating each of the three chips separately, weighted by the inverse flux errors provided \apogee\ fits files. A 2nd-order Chebyshev polynomial is sufficient to use for the pseudo-continuum normalised data provided by \apogee\ and we apply this normalisation to both \aspcapstar\ and  \apstar\ files. Treating the three chips separately, we fit the polynomials over the wavelength regions of (i) 15150-15800 $\AA$, (ii) 15890 - 16430 $\AA$ and (iii) 16490 - 16950 $\AA$. \textbf{MKN Keep going with this boarding now annoying}.
...
First, we adopt the DR10 ``\aspcap\ corrected'' stellar parameters for each of the reference objects as their labels, in order to place the output of \tc 's test step for the survey objects on the \apogee\ \aspcap\ scale. These \aspcap\ corrected labels are available for all stars except the 65 dwarf stars from the Pleiades cluster. 
...
\end{eqnarray}
where $\set{\theta}_\lambda$ is the set of spectral parameters at each $\lambda$.  Each element of $\starlabelvec_n$ can be some (possibly complicated) function of the full set of $K$ labels, $\starlabelvec_n$, which
...
This model assumes that the noise is Gaussian, zero mean,
and independent for every measurement.
That is, the noise model is
...
a data-driven probabilistic generative model model for stellar spectra from the reference objects (training data).
...
spectra.
These untagged spectra---the survey objects---will be referred to as the ``test data'' sometimes in
what follows.
