...
\textcolor{blue}{ \tc\ is similar to the MATrix Inversion for Spectral SythEsis (\matisse) and \textit{ULySS} procedure
...
that we can test exactly how well we can reproduce the results from \apogee\ for the survey stars via label-transfer from only 543 stars. \textcolor{blue}{A limitation of this reference set of objects is that main sequence stars are not well sampled, which as we discuss in Section 5, limits our ability to determine labels for these stars at the test step. }
...
\textcolor{blue}{To demonstrate the fit of the model to the \apogee\ spectra at test time, using the continuum normalised test stars shown in Figure 3, we plot the best fit model and with the corresponding spectra for the regions A and B highlighted inFigure 3, which demonstrates the model is an excellent fit to the data. \textbf{DWH is there anything else you want to say here?} }


\begin{figure}[!h]
\centering
 \includegraphics[scale=0.4]{./plots/aftersubmit/model_data.png}
\caption{\textcolor{blue}{The four stars in Figure 3 across narrow wavelength intervals A and B, as described in Section 2.3. The stars are plotted in black with the models in red, including the span of the scatter of the fit, generated by \tc.}}
\label{fig:modeldata}
\end{figure}
...
\tiny{21360941+4212409} & 4840.2  & 2.60 & -0.48  & 12.6 &  0.027 &  0.013 &  1.23  & 0.0 &  0.0 &  142608608 &  0 \\
 \hline
\end{tabular}
\label{tab:online} }
\end{table*}  
...
\centering
  ...
  \hspace{-20pt}
    \includegraphics[scale=0.26]{./plots/aftersubmit/fig8b.png}
...
\begin{figure}[!h]
\centering
 \includegraphics[scale=0.26]{./plots/aftersubmit/fig9a.png}
  \hspace{-20pt}
    \includegraphics[scale=0.26]{./plots/aftersubmit/fig9b.png}
...
derived them for essentially all of the 55,000 DR10 survey stars, based on a training step that involved only 543 reference objects, i.e. 1\% of the survey.  Remarkably, \tc 's label transfer results in stellar parameters and metallicites that are as precise and accurate as those derived from
