...
Fundamentally, \tc\ (in its simplest form; this could be generalized)
expects all spectra---reference and survey spectra---to be continuum normalized in a consistent way,
and sampled on a consistent rest-frame wavelength grid.
It further assumes that every spectrum is subject to the a similar line-spread function,
and that the noise variance is known at each specral pixel of each spectrum.
...
are very signal-to-noise dependent, because pixels that are clearly \emph{not} continuum in high signal-to-noise
spectra are completely consistent with being continuum at lower signal-to-noise.
Therefore, to make \tc\ as independent of signal-to-noise (hereafter ``S/N'') as possible,
we base the continuum estimation on a pre-tabulated set of wavelength locations that we know
(from running \tc\ itself iterativel) are not strongly affected by absorption lines.
This constitutes a data-driven method for finding continuum pixels.
We find that this method 
leads to very small systematic dependence of the spectra on S/N.
...
That is, we can determine with the \tc\ the `true' continuum, using the model derived from the pseudo-continuum normalised spectra for the reference objects provided by \apogee, as described in \sectionname~\ref{sec:results}.

Given the list of continuum pixels found by this procedure,
we determine the continuum by fitting a low-order Chebyshev polynomial to these continuum pixels,
...
This \figurename\ illustrates typical \apogee\ spectra and demonstrates how the spectra 
vary as a function of metallicity at a given temperature , and as a function of temperature at a given metallicity. 
For a clearer view of individual absorption linea features, we use narrower regions marked in this \figurename, (A) and (B), for all subsequent examination of the spectral data. 
...
MKN:  THE FOLLOWING PARAGRAPH (below) NEEDS ITS OWN SUBSECTION, SINCE IT HAS NOTHING TO DO WITH CONTINUUM.
ALSO, THIS SAYS NOTHING REALLY ABOUT WHAT WE ARE DOING.
IT SHOULD SAY WHERE WE GET OUR INVVAR VALUES FROM AND HOW WE ADJUST THEM IF AT ALL.
THE POINT OF ALL OUR TEXT SHOULD BE TO SAY HOW WE TOOK THE \apogee\ DATA AND CONVERTED IT TO PROPER \tc\ INPUTS.
...
MKN:  THIS SUBSECTION TITLE MAKES NO SENSE TO ME; ISN'T THIS THE SUBJECT OF THIS WHOLE SECTION?

\subsubsection{Apogee Spectra}\label{sec:ApogeeSpectra}

MKN:  THIS SUBSUBSECTION TITLE MAKES EVEN LESS SENSE; ISN'T THIS THE SUBJECT OF THIS WHOLE SECTION?
...
MKN:  I THINK YOU CAN DELETE THE NEXT PARAGRAPH?

In the following sections we demonstrate that \tc\ reproduces \apogee\ \teff, \logg\ and \feh\ using a npumber of selected fields across the bulge, disk and halo with very small intrinsic errors which are comparable to \aspcap\ intrinsic errors at lower SNR than minimisation techniques. 
...
MKN:  YOU HAVE TO DECIDE BETWEEN ``S/N'' and ``SNR''.  THEN AUDIT AND FIX IT ALL.
...
The spectral model is based on the notion that the continuum-normalized spectra of
stars with identical labels look near-identical at every pixel, save for the observational errors
and some intrinsic scatter; and that the expected flux at every pixel changes continuously
...
\end{eqnarray}
where $\set{\theta}_\lambda$ is the set of spectral parameters at each $\lambda$.  Each element of $\starlabelvec_n$ can be some (possibly complicated) function of the full set of $K$ labels, $\set{l}_n$, which
...
where $\xi_{n\lambda}$ is a Gaussian random number with zero mean and unit
variance.
...
\begin{eqnarray}
\ln p(f_{n\lambda}\given\set{\theta^T}_\lambda, \starlabelvec_n, s_\lambda^2) &=&
...
labels $\starlabel_{nk}$ in the label vector, $\starlabelvec_n$.
...
\begin{eqnarray}
\set{\theta_\lambda},s_\lambda \leftarrow \substack{\mbox{argmax}\\{\theta_\lambda}}
\sum_{n=1}^N \ln p(f_{n\lambda}\given\set{\theta^T}_\lambda, \starlabelvec_n, s_\lambda^2)
...
\starlabelvec_n &\equiv& [1,
                          ...
                          \cdots,
                          (\starlabel_{nK} - \bar{\starlabel_K})\,(\starlabel_{nK} - \bar{\starlabel_K})]
\label{eq:quadinlabels}\quad ,
\end{eqnarray}
where the leading ``1'' permits an offset, and the quadratic terms
contain all possible products exactly once.
...
$s_\lambda^2$ is a pure linear-algebra operation (weighted least
squares), expect that $\starlabelvec_n$ has become longer for a given number of labels. 
...
\begin{eqnarray}
\starlabelvec_m \leftarrow \substack{\mbox{argmax}\\{\starlabelvec_m}}
\sum_{\lambda=1}^{N_\pix}
\ln p(f_{m\lambda}\given\set{\theta^T}_\lambda, \starlabelvec_m, s_\lambda^2)
\label{eq:teststep}
\end{eqnarray} 
...
mean model $\starlabelvec_m$, the optimization to obtain maximum-likelihood labels
...
becomes  
\begin{equation}
\starlabelvec_n\equiv(\teff\  , \logg\  , \feh\  , \teff^2\ , \teff\cdot \logg\ , \teff \cdot\feh\ , \logg^2 \ , \logg\cdot \feh\  , \feh^2)
