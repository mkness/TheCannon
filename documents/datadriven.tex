...
% # style notes
% - are they ``tags'' or ``labels''?  MKN:  Make a call and audit the whole text.
...
An optimal exploitation of all of these datasets relies on their homogenisation, e.g. a standard stellar parameter scale for stars that have been observed by the Northern and Southern surveys, that operate across different wavelength regions. The facilitator of this effort is cross over calibration stars observed between surveys. These may be stars studied and characterised at high resolution in the optical wavelength region and also globular and open clusters which have well determined labels. These calibration stars afford a critical opportunity to create a data driven model, which can be applied across \textit{all surveys} via label transfer, given known or adopted labels of \teff\, \logg\, \feh\ from an adopted calibration set of stars. This set of stars must be observed across all surveys and must have an adopted scale of parameters and individual abundance, so called labels, which can be directly transferred from these stars to all stars in a survey.  \\

There are therefore also numerous opportunities in this era of large datasets. For the first time, using the data itself, we have the opportunity not to simply find a best fitting spectrum, but to fully characterise spectra and understand how at each data point, the flux is a function of the labels. 
...
We implement two tests with this training data. We firstly adopt the ``ASPCAP corrected'' stellar parameters of each of these stars in the training set as our tags for the stars; this nominally ensures that we are on the same calibration as APOGEE for stellar parameters. The corrections made by APOGEE for the cluster data are applied to the direct output of the ASPCAP pipeline based on a temperature calibration to the infrared flux method \citep{Gonzalez2009} ,a \logg\ correction from the offset between ASCAP results and Kepler results for common stars and [Fe/H] offsets from the output of ASCAP versus the literature value of each cluster.  The APOGEE corrections that they determine are valid only for stars with \logg\ $<$ 3.5 and are not implemented for the dwarfs. ASPCAP provides [M/H] values for these clusters, that are corrected to the [Fe/H] of the clusters and here we adopt this tag as an [Fe/H] - it does not specifically and explicitly use [Fe/H] lines, but this is derived from an [Fe/H] correction. 

These corrections place the giants in the cluster stars on or near the iscohrones (see Figures 7 and 8 in Meszaros et al., (2013)). There are no ASPCAP corrections implemented for the dwarf spectra. Instead, we determine temperatures for the dwarfs, which are all in the Pleaides, directly using the infrared flux value of \citep{Gonzalez2009} and set all [Fe/H] of the dwarf spectra to [Fe/H] = 0.03 (reference). To determine the log g for these stars, we shift the stars vertically to their nearest positions on at 150 Myr Padova isochrone at [Fe/H] = 0.03. Due to the high differential reddening to the Pleaides, and the subsequent large temperature errors using the IR flux method that result from this, we only selected the 50 from a total of 75 Pleiades dwarfs with SFD E(J-K) values of < 0.12. 

Assuming the input labels from the ASPCAP corrected parameters determined from calibrations to literature cluster values also wraps in the errors from the ASPCAP pipeline: of XX in Teff, XX in \logg\ and XX in [Fe/H], but does place \tc directly on the ASPCAP scale for comparison and analysis of the label transfer method.

We also for comparison, implement our own training set calibration; assuming a more basic characterisation of labels. Here we adopt temperatures from the IR flux method of XX directly, take \logg\ from the best vertical fits to the isochrone (with the temperatures fixed) and assume the metallicity of each star in the cluster is the same, at the mean metallicity of the cluster assuming the literature values of the cluster. We adopt a different literature value of M35(check) compared to APOGEE as this cluster appears to fit better at the value of XX according to the model and there is variation in the literature on this metallicity ranging from our adopted value of -0 35 to x. 
