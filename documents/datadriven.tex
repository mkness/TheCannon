...
\begin{eqnarray}
y_{n\ell} &=&
 Y(\setof{x_{nk}}_{k=1}^K\given\theta_\ell) + \sqrt{s_\ell^2 + \sigma_{n\ell}^2}\,\xi_{n\ell}
\label{eq:model}\quad ,
\end{eqnarray}
where $Y()$ is some (possibly complicated) function of the full set
of $K$ tags, $\theta_\ell$ is the set of parameters controlling the
...
\begin{eqnarray}
\ln p(y_{n\ell}\given\theta_\ell,s_\ell^2) &=&
 -\frac{1}{2}\frac{[y_{n\ell} - Y(\setof{x_{nk}}_{k=1}^K\given\theta_\ell)]^2}{s_\ell^2 + \sigma_{n\ell}^2}
...
$[\theta_\ell,s_\ell^2]$ inferred independently, and all the
individual-spectra log-likelihoods can be added together to make
\begin{eqnarray}
\ln p(\setof{y_{n\ell}}_{n=1}^N\given\theta_\ell,s_\ell^2) &=&
 \sum_{n=1}^N \ln p(y_{n\ell}\given\theta_\ell,s_\ell^2)
\label{eq:like}\quad.
\end{eqnarray}
We can set the parameters $[\theta_\ell,s_\ell^2]$ either by
...
$\theta_\ell$ and the scatter $s_\ell^2$ as free parameters, and the
tags $x_{nk}$ as fixed.
...
function parameters $\theta_\ell$ and scatter $s_\ell^2$ as fixed and
...
linear in the tags:
\begin{eqnarray}
Y(\setof{x_{nk}}_{k=1}^K\given\theta_\ell) &=& a_{\ell 0} + \sum_{k=1}^K a_{\ell k}\,[x_{nk} - \mean{x_k}]
\\
\theta_\ell &\equiv& \setof{a_{\ell k}}_{k=0}^K
\quad ,
\end{eqnarray}
...
$y_{m\ell}$ at each of many wavelengths $\lambda_\ell$, and an
...
handled by setting variances to infinity.
...
being functions not of the function parameters $\theta_\ell$ and
scatter $s_\ell^2$ but instead as functions of the \emph{tags}
$x_{mk}$:
\begin{eqnarray}
\ln p(y_{m\ell}\given\setof{x_{mk}}_{k=1}^K) &=&
 -\frac{1}{2}\frac{[y_{m\ell} - Y(\setof{x_{mk}}_{k=1}^K\given\theta_\ell)]^2}{s_\ell^2 + \sigma_{m\ell}^2}
 -\frac{1}{2}\ln(s_\ell^2 + \sigma_{m\ell}^2)
 -\frac{1}{2}\ln 2\pi
\\
\ln p(\setof{y_{m\ell}}_{\ell=1}^L\given\setof{x_{mk}}_{k=1}^K) &=&
 \sum_{\ell=1}^L \ln p(y_{m\ell}\given\setof{x_{mk}}_{k=1}^K)
\label{eq:taglike}\quad,
\end{eqnarray}
where the likelihood functions are given as a function of tags now,
and the sum is now over the full set of $L$ wavelengths
$\lambda_\ell$.
These likelihood functions effectively assume that the function
parameters $\theta_\ell$ and scatters $s_\ell^2$ are all known (from
the training data).
