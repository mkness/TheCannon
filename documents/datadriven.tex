...
In most cases, it suffices to think of the labels as \teff , \logg, and the element abundances [X/H], though stellar rotation, microturbulence, age, and so forth can also be thought of as labels.
...
Our most basic implementation of \tc\ that we present includes only three labels, but this can easily be extended to additional labels  (for example, [$\alpha$/Fe], [X/Fe]) and also more comprehensive models (for example, Gaussian processes). 
...
In principle, \tc\ is applicable to any large, homogeneous spectroscopic data set.
...
In practice, a set of reference objects that satisfies these criteria (for example, in \apogee)
...
That is, we can determine with the \tc\ the `true' continuum, using the model derived from the pseudo-continuum normalised spectra for the reference objects provided by \apogee, as described in Section \ref{sec:results}. Subsequently, we fit a low-order Chebyshev polynomial to these continuum pixels,
...
From the coefficients of an initial fit of this spectral models (see, for example, the middle panel of Fig.~\ref{fig:coeffs}), 
...
(cf. \S~\ref{sec:ContNorm}). Specifically, we found that 35\% of the pixels in the baseline spectrum of the model (the vector $\theta^0_\lambda$) flux levels within 1\% of unity. However, not all these pixels are suitable continuum pixels, as many of them have significant dependencies (that is, $\theta^{1,2,3}_\lambda$)
on the three labels. 
...
We take the spectral model built in the \apogee\ cluster stars in \S~\ref{sec:ModelComplexity}, \S~\ref{sec:take-one-out} and \S~\ref{sec:ApogeeContinuum},
