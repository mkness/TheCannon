...
We have demonstrated that mathematically, \tc\ works and can return labels of \teff, \logg, \feh, \alphafe\ and mass for \apogee\ spectra. We have shown where the information is contained in the stellar spectrum that most informs each of these labels and interpreted this information in chemical abundance space. Most importantly, that the mass information is derived from weak CN regions. This is an astrophysically plausible origin of the mass dependence given expectations from stellar evolution and the reported changes in these elements from dredge-up mixing processes, as the star moves along the red giant branch. 
...
The red clump sample show that age decreases in x and y and show that young tight and old not tight - learn from this inside out and mixing timescales. We provide a catalogue in table X (reproduced in full online) with masses and ages for all apogee red giants in label space of training data. 

\ldots DWH: Emphasis on the reason that the science result is so
convincing that we really have an age indicator here.

PUT IN TABLE WITH FULL CATALOGUE HERE OF XXXX STARS

\ldots MKN: How do you obtain our code, data, and results? github here: to run on apogee spectra and obtain 5 labels. straightforward to change to other wavelength regions and training sets. 
...
\Idots MKN write some conclusions after a break from writing. 
