...
There are a handful of known spectral mass indicators. These derive from rotation, in the line-profile variations due to non-LTE effects, as convection scales with mass, and from chromospheric emission from a time-dependent magnetic field e.g. The Ca II HK and H$\alpha$ (see Soderblom 2010 for a review). Furthermore, they decay of long lived isotopes like Th, which can be measured at 4019 Angstroms, can be used for age dating by comparing its abundance to that of the stable elements in the stellar photosphere. Commonly, age estimates are determined for the sub-giant class of stars observed at high resolution using interpolation between stellar models.  (e.g. Bensby2013, Casagrande2011, Haywood20XX). This region of the \teff-logg\ plane is the least insensitive to age. However, the age constraints using this method are typically weak or may only provide an upper limit on the age estimate. Up until now the Geneva Cophenhagen survey of stars in the solar neighbourhood (Nordstršm et al. 2004), with a sample of about 16,682 stars has been the largest sample of stars, all nearby main sequence stars, with complete data on stellar ages in the Milky Way disk. These ages have been determined given the measured parallaxes to these stars, as well as their stellar parameters of \teff, \logg\ and \feh\ and interpolation between stellar isochrone models \citet{Holm2009} to infer the age. 
...
Importantly, mass and therefore inferred age might only appear in the spectra as
...
For a review on ages of stars \citep{Soderblom2010}.
